    ...
    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-6">
      ...
      <section class="mb-8">
        ...
        <div class="space-y-4">
          ...
          <!-- Подпись -->
          <div class="relative">
            <label class="block text-gray-700 font-medium mb-2">Подпись (максимум 11 символов)</label>
            <textarea
              v-model="signature"
              class="w-full px-4 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-300"
              rows="3"
              :maxlength="11"
              @input="handleInput"
              :class="{
                'border-orange-500': remainingCharacters <= 3 && remainingCharacters > 0,
                'border-red-500': remainingCharacters === 0
              }"
            ></textarea>
            <!-- Счётчик символов -->
            <p 
              class="absolute bottom-1 right-2 text-sm"
              :class="{
                'text-gray-500': remainingCharacters > 3,
                'text-orange-500': remainingCharacters <= 3 && remainingCharacters > 0,
                'text-red-500': remainingCharacters === 0
              }"
            >
          ...
          <div class="mt-6 flex justify-end space-x-4">
            ...
            <button
              @click="saveSignature"
              class="bg-purple-600 text-white font-semibold px-6 py-2 rounded-lg shadow-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-400 transition-colors duration-200"
            >
              Сохранить
            </button>
          </div>
