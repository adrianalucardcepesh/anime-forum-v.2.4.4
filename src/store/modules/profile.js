...
const actions = {
  ...
  async updateAvatar({ commit, dispatch }, { userId, avatarFile }) {
    try {
      ...
      const uploadResponse = await axios.post(`http://95.164.90.115:3000/upload/image/${userId}`, formData, {
        headers: { 
      ...
      if (uploadResponse.data && uploadResponse.data.fileUrl) {
        ...
        const avatarUrl = uploadResponse.data.fileUrl.replace(
          'http://localhost:3000',
          'http://95.164.90.115:3000'
        );

        console.log('Сохраняем URL в Firebase:', avatarUrl);
        // Обновляем профиль в Firebase с новым URL аватара
        const avatarRef = databaseRef(database, `users/${userId}/profile/avatarUrl`);
